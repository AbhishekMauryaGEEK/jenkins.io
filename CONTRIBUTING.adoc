...
* You can't verify changes locally before creating the PR.
* It only works on one file at a time.
* Your changes are lost if you close or navigate away from the page.
...
For example, the changelog for link:https://jenkins.io/changelog/#v2.396[Jenkins weekly 2.396] includes a bug fix with such an exception.

==== Adding a Solution page

Solution pages are somewhat *special* because they are generally not AsciiDoc files, but rather link:https://haml.info[Haml] templates.
...
* link:https://haml.info[Haml] for more advanced/custom page creation.
** link:https://haml.info/docs/yardoc/file.REFERENCE.html[Haml syntax reference]
